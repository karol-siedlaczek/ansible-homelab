...
my @expressions_netsnmpmem = (
        "total_free=snmp(1.3.6.1.4.1.2021.4.11.0),1024,/,round(1),' MiB',+",    # Total free, data is reported in kb, we want MB
        "total_real=snmp(1.3.6.1.4.1.2021.4.5.0),1024,/,round(1),' MiB',+",     # Total real memory
        "avail_real=snmp(1.3.6.1.4.1.2021.4.6.0),1024,/,round(1),' MiB',+",     # Free real memory
        "total_swap=snmp(1.3.6.1.4.1.2021.4.3.0),1024,/,round(1),' MiB',+",     # Total swap
        "avail_swap=snmp(1.3.6.1.4.1.2021.4.4.0),1024,/,round(1),' MiB',+",     # Free swap, here "MB" is added to the end as a string
        "min_swap=snmp(1.3.6.1.4.1.2021.4.12.0),1024,/,round(1),' MiB',+",
        "shared=snmp(1.3.6.1.4.1.2021.4.13.0),1024,/,round(1),' MiB',+",        # In use shared memory (should be 0 on > 2.4 kernel)
        "buffer=snmp(1.3.6.1.4.1.2021.4.14.0),1024',/,round(1),' MiB',+",       # In use buffer memory
        "cached=snmp(1.3.6.1.4.1.2021.4.15.0),1024,/,round(1),' MiB',+",                # In use cached memory
        "total=total_real,total_swap,+,' MiB [',.,total_real,.,' real ',.,total_swap,.,' swap : ',.,total_free,.,' free]',.",  # Total memory on the system (you can still use it as a number since everything after first few digits would be cut of for numeric calculations, also only number is reported in perf data; I should probably impliment printf function to make formatting and output easier...)
        ...
        'Swap usage=used_swap,total_swap,%',                                    # Percent of swap memory in use
        'cached_buffer=cached,buffer,+',
        '%cached_buffer=cached_buffer,total_real,%',
        'cached_buffer_shared=cached_buffer,shared,+',
        '%cached_buffer_shared=cached_buffer_shared,total_real,%',
        'cached_buffer_shared_free=cached_buffer_shared,total_free,+',
        'free_mem=avail_real,cached_buffer_shared,+',
        'used_mem=total_real,free_mem,-',
        'Mem usage=used_mem,total_real,%',
...
use lib "/usr/local/bin/nagios/checks";
